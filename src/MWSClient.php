...
class MWSClient
{
     ...
     * @return array
     */
    public function postProduct($MWSProduct, $template = 'Custom', $version = null,$Category = null, $signature = null)
    {
        ...
        $csv->insertOne(['TemplateType=' . $template, 'Version=' . $version,'Category='.$Category, 'TemplateSignature=' . $signature]);
    ...
    public function GetReport($ReportId)
    {
        ...
        if ($status !== false && $status['ReportProcessingStatus'] === '_DONE_NO_DATA_') {
            ...
            if (is_string($result)) {
                $isJp = in_array($this->config['Marketplace_Id'], ['AAHKV2X7AFYLW', 'A1VC38T7YXB528']) ?
                    true : false;
                ...
                if ($isJp){
                    foreach ($headers as &$v){
                        $v = mb_convert_encoding($v, 'UTF-8', 'Shift-JIS');
                    }
                }
                $result = [];
